...
func createElasticCacheSession(region *string, roleArn string) *ec.ElastiCache {
	sess, err := session.NewSession()
	if err != nil {
		panic(err)
	}
	...
	return ec.New(sess, config)
}

func createRdsSession(region *string, roleArn string) *rds.RDS {
	sess, err := session.NewSession()
	if err != nil {
		panic(err)
	}
...
func (iface tagsInterface) get2(job job) (resources []*tagsData, err error) {
	if job.Type == "ec" {
		input := &ec.DescribeCacheClustersInput{}
		ctx := context.Background()
		pageNum := 0
		return resources, iface.ecSvc.DescribeCacheClustersPagesWithContext(ctx, input,
			func(page *ec.DescribeCacheClustersOutput, lastPage bool) bool {
				pageNum++
				for _, row := range page.CacheClusters {
					resource := tagsData{
						ID:      row.CacheClusterId,
						Service: &job.Type,
						Region:  &job.Region,
					}
					resources = append(resources, &resource)
				}
				return true // 必须获取所有结果
			})
	} else if job.Type == "rds" {
		input := &rds.DescribeDBInstancesInput{}
		ctx := context.Background()
		pageNum := 0
		return resources, iface.rdsSvc.DescribeDBInstancesPagesWithContext(ctx, input,
			func(page *rds.DescribeDBInstancesOutput, lastPage bool) bool {
				pageNum++
				for _, row := range page.DBInstances {
					resource := tagsData{
						ID:      row.DBName,
						Service: &job.Type,
						Region:  &job.Region,
